...
const getRoomStatus = (name) =>
  request(`v0/room/${name}/status`).then(convertStatus);

const setRoomMode = (name, mode) =>
  request(`v0/room/${name}/mode/${mode}`, "PUT");

const setRoomHeat = (name, temp) =>
  request(`v0/room/${name}/heat/temp/${temp}`, "PUT");
